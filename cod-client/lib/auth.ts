...
export async function getUserRole(): Promise<"admin" | "staff" | "super_admin" | null> {
  const user = await getUser();
...
export async function requireAdmin(): Promise<void> {
  const user = await getUser();
  if (user?.role !== "admin" && user?.role !== "super_admin") throw new ForbiddenError("هذه العملية متاحة للمسؤولين فقط");
}

export async function requireSuperAdmin(): Promise<void> {
  const user = await getUser();
  if (user?.role !== "super_admin") throw new ForbiddenError("هذه العملية متاحة لمسؤولي المنصة فقط");
}
