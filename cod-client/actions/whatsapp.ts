...
export async function listWhatsAppMessages(filters: WhatsAppFilters = {}): Promise<{ messages: WhatsAppMessage[]; count: number }> {
...
export async function sendWhatsAppMessage(data: SendWhatsAppData): Promise<{ id: string; status: string }> {
  ...
  try {
    ...
    if (error instanceof ApiClientError && error.code) {
      ...
      console.error("[WhatsApp Action Error]", {
        code: error.code,
