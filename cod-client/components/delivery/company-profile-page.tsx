    ...
    <>
      <div className="space-y-6 animate-fade-in">
        ...
        <div className={cn(
          ...
          {isConnected && <div className="h-[3px] bg-border" />}
          <div className="p-6 flex flex-col sm:flex-row sm:items-center gap-5">
            {/* Logo */}
            <div className={cn(
              "w-16 h-16 rounded-lg flex items-center justify-center shrink-0 text-2xl font-semibold shadow-md",
              isConnected
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 flex-wrap">
                <h1 className="text-2xl font-semibold text-foreground tracking-tight">{config.name}</h1>
                <span className={cn(
              ...
              <div className="flex flex-wrap gap-1.5 mt-3">
                {config.supportsHomeDelivery && (
                  <span className="px-2.5 py-1 rounded-lg bg-primary/5 text-primary text-[10px] font-semibold uppercase tracking-wider flex items-center gap-1">
            ...
            <ProtectedAction userScopes={userScopes} requiredScope={SCOPES.DELIVERY_MANAGE}>
              <Button
                ...
                className="h-11 rounded-xl text-[11px] font-semibold uppercase tracking-[0.15em] active:scale-95 shrink-0 w-full sm:w-auto"
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
          ...
          <div className="lg:col-span-2 space-y-5">
            {supportsWebhook && (
              <button
                ...
                className={cn(
                  "w-full bg-card rounded-lg border-border p-5 text-start transition-all group",
                  isConnected ? "hover:border-primary/15 hover:shadow-sm active:scale-[0.995] cursor-pointer" : "opacity-50 cursor-default",
                  isConnected && hasWebhook && "border-success/30"
                )}
              >
                <div className="flex items-center justify-between gap-4">
                  <div className="flex items-center gap-3">
                    <div className={cn(
                      ...
                      hasWebhook ? "bg-success/10" : "bg-muted/30"
                    )}>
                      <Zap className={cn("w-4.5 h-4.5", hasWebhook ? "text-success" : "text-muted-foreground/30")} />
                    </div>
                    <div>
                      <p className="text-sm font-semibold text-foreground/80">{t.webhook?.title ?? "Real-time Updates"}</p>
                  ...
                  <div className={cn(
                    ...
                    !isConnected ? "bg-muted-foreground/20" : hasWebhook ? "bg-success" : "bg-muted-foreground/20"
                  )} />
                </div>
              </button>
            )}
          ...
          {config.supportsStopDesk && (
            <div className="lg:col-span-1">
              <ProtectedAction userScopes={userScopes} requiredScope={SCOPES.DELIVERY_MANAGE}>
                <div className={cn(
                  "bg-card rounded-lg border-border p-5 h-full transition-opacity",
                  ...
                  <div className="flex flex-col gap-2">
                    <button
                      ...
                      className="flex items-center gap-3 w-full h-11 px-4 rounded-xl border border-border bg-muted/10 hover:bg-primary/5 hover:border-primary/15 text-muted-foreground hover:text-primary text-[11px] font-semibold uppercase tracking-wide transition-all active:scale-95 disabled:opacity-40 disabled:pointer-events-none text-start"
                    >
        ...
        {company && (
          <ProtectedAction userScopes={userScopes} requiredScope={SCOPES.DELIVERY_MANAGE}>
            <div className="bg-card rounded-lg border-border p-4 sm:p-5">
              ...
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-2.5 min-w-0 flex-1">
                  <div className={cn(
                    ...
                    autoValidate ? "bg-warning/10" : "bg-success/10"
                  )}>
                    {autoValidate
                      ? <ShieldAlert className="w-4 h-4 sm:w-[18px] sm:h-[18px] text-warning" />
                      : <ShieldCheck className="w-4 h-4 sm:w-[18px] sm:h-[18px] text-success" />}
                  </div>
                  <p className="text-[13px] sm:text-sm font-semibold text-foreground/80 truncate">
              ...
              {isEcotrackProvider && (
                <div className="mt-4 rounded-xl border border-warning/30 bg-warning/5 p-3 flex items-start gap-2.5">
                  <ShieldAlert className="w-3.5 h-3.5 text-warning shrink-0 mt-0.5" />
                  <p className="text-[11px] sm:text-xs font-semibold text-warning dark:text-warning leading-relaxed">
