...
export function ProductsTable({ products, groups, loading = false, onView, onEdit, onDelete, onCreate }: Props) {
  ...
  const columns: TableColumn<Product>[] = [
      ...
      render: (value, row) => (
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            {row.sku && <p className="text-xs text-muted-foreground truncate font-mono">{row.sku}</p>}
          </div>
        </div>
      ),
    },
    {
      ...
      isSubtitle: true,
      render: (value) => value ? (
        <Badge variant="secondary" className="text-xs font-medium py-0 h-5">
          {groupMap[value]?.name ?? "—"}
        </Badge>
    ...
    },
    {
      ...
      sortable: true,
      render: (value) => (
        <p className="font-semibold text-sm text-foreground">{formatPrice(value, common.currency.symbol)}</p>
      ),
    },
    {
      key: "variantsCount",
      ...
      render: (_, row) => {
        ...
        return (
          <div className="flex items-center gap-1">
            ...
            {row.avgRating != null && (
              <span className="text-sm font-medium text-foreground tabular-nums">{row.avgRating}</span>
            )}
      ...
      renderMobileCard={(product) => (
        <div className="space-y-4 py-0.5">
          ...
          <div className="flex items-start gap-3.5">
            <div className="w-14 h-14 bg-muted rounded-lg flex items-center justify-center overflow-hidden shrink-0 border border-border">
            ...
            {(onView || onEdit || onDelete) && (
              <div className="shrink-0">
                <DropdownMenu>
                  <DropdownMenuTrigger render={<button className="w-9 h-9 rounded-md bg-muted text-muted-foreground hover:text-foreground flex items-center justify-center transition-colors" />}>
                    <MoreHorizontal className="w-4 h-4" />
                  </DropdownMenuTrigger>
                  <DropdownMenuContent align={dir === "rtl" ? "start" : "end"} className="rounded-md border-border bg-popover text-popover-foreground">
                    {onView && (
                      <DropdownMenuItem onClick={() => onView(product)} className="text-sm py-2.5">
                        <Eye className="w-3.5 h-3.5 me-2" />{t.actions.view}
                      </DropdownMenuItem>
                    )}
                    {onEdit && (
                      <DropdownMenuItem onClick={() => onEdit(product)} className="text-sm py-2.5">
                        <Edit className="w-3.5 h-3.5 me-2" />{t.actions.edit}
                      </DropdownMenuItem>
                    )}
                    {onDelete && (
                      <DropdownMenuItem onClick={() => onDelete(product)} className="!text-destructive text-sm py-2.5">
              ...
              <div className={cn("grid gap-3 pt-3.5 border-t border-border/10", gridClass)}>
                ...
                {(product.variantsCount ?? 0) > 0 && (
                  <div className="space-y-1">
                    ...
                    <p className="text-sm font-medium text-foreground">{product.variantsCount}</p>
                  </div>
                )}
                  ...
                  <div className="space-y-1">
                    ...
                    <div className="flex items-center gap-1 mt-0.5">
                      ...
                      {product.avgRating != null && (
                        <span className="text-sm font-medium text-foreground tabular-nums">{product.avgRating}</span>
                      )}
