  ...
  const columns: TableColumn<ProductCategory>[] = [
    {
      ...
      render: (value, row) => (
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 bg-muted rounded-lg flex items-center justify-center shrink-0 border border-border/200 shadow-sm overflow-hidden">
            {row.imageUrl ? (
    ...
    },
    {
      ...
      isSubtitle: true,
      render: (value) => value ? (
        <Badge variant="secondary" className="text-[10px] font-semibold uppercase tracking-widest py-0 h-5">
          {groupMap[value]?.name ?? "—"}
        </Badge>
    ...
    },
    {
      ...
      sortable: true,
      render: (value) => <span className="text-sm font-semibold text-foreground/80 tabular-nums">{value ?? 0}</span>,
    },
    {
      key: "createdAt",
      ...
      renderMobileCard={(group) => (
          <div className="space-y-4 py-0.5">
            <div className="flex items-center gap-3.5">
              <div className="w-12 h-12 bg-muted rounded-xl flex items-center justify-center shrink-0 border border-border/200 shadow-sm overflow-hidden">
                {group.imageUrl ? (
              ...
              <div className="flex-1 min-w-0">
                <p className="font-semibold text-[15px] text-foreground tracking-tight truncate leading-tight">{group.name}</p>
              ...
              <div className="flex flex-col items-end gap-2 shrink-0">
                <div className="bg-primary/5 border border-primary/10 rounded-lg px-2 py-1 flex flex-col items-center min-w-12">
                  <span className="text-[10px] font-semibold text-primary tabular-nums">{group.productsCount ?? 0}</span>
            ...
            <div className="flex items-center justify-between pt-3 border-t border-border/40">
              <div className="flex items-center gap-2">
                {group.parentId && (
                  <div className="flex items-center gap-1.5 px-2 py-1 rounded-lg bg-muted/50 text-[10px] font-semibold text-muted-foreground uppercase tracking-widest">
              ...
              <div className="flex items-center gap-2">
                <Button 
                  ...
                  variant="outline" 
                  size="sm" 
                  className="h-8 rounded-lg border-border bg-card text-[10px] font-semibold uppercase tracking-widest active:scale-95"
                >
                ...
                <Button 
                  ...
                  variant="outline" 
                  size="sm" 
                  className="h-8 rounded-lg border-destructive/30 bg-destructive/5 text-destructive text-[10px] font-semibold uppercase tracking-widest active:scale-95 disabled:opacity-30"
                >
