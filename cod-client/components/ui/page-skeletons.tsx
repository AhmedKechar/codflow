...
export function ListHeaderSkeleton({ showAction = true }: { showAction?: boolean }) {
  return (
    <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
      ...
      {showAction ? <Skeleton className="h-9 sm:h-10 w-32 sm:w-40" /> : null}
    </div>
  );
}
    ...
    <>
      ...
      <div className="sm:hidden space-y-3">
        {[...Array(mobileCards)].map((_, i) => (
          <div key={i} className="bg-card rounded-lg border border-border/40 p-5 space-y-4">
...
export function CardGridSkeleton({
  ...
  return (
    <div className={`grid ${colClass} gap-4`}>
      {[...Array(cards)].map((_, i) => (
        <div key={i} className="bg-card rounded-lg border border-border/40 overflow-hidden p-5 space-y-4">
      ...
      <div className={`grid grid-cols-1 ${showSidebar ? "md:grid-cols-12" : ""} gap-6`}>
        <div className={`${showSidebar ? "md:col-span-8" : ""} space-y-6`}>
          {[...Array(sections)].map((_, s) => (
            <div key={s} className="bg-card rounded-lg border border-border/40 overflow-hidden">
        ...
        {showSidebar ? (
          <div className="md:col-span-4 space-y-6">
            <div className="bg-card rounded-lg border border-border/40 p-6 space-y-4">
              <Skeleton className="h-4 w-24" />
              <div className="space-y-2">
                <Skeleton className="h-3 w-20" />
                <Skeleton className="h-12 w-full" />
...
export function DetailViewSkeleton({
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-card rounded-lg border border-border/40 p-6 md:p-8">
        <div className="flex flex-col lg:flex-row gap-6">
          ...
          {showStats ? (
            <div className="grid grid-cols-2 lg:grid-cols-1 gap-3 shrink-0 lg:w-56 w-full">
              ...
              <Skeleton className="h-20" />
              <Skeleton className="h-20 col-span-2 lg:col-span-1" />
            </div>
          ) : null}
        </div>
      </div>
        ...
        <div key={s} className="space-y-4">
          ...
          <div className="bg-card rounded-lg border border-border/40 overflow-hidden">
            {[...Array(3)].map((_, i) => (
              <div
                key={i}
                className="flex items-center justify-between p-6 border-b border-border/40 last:border-0"
              >
                <div className="flex items-center gap-3">
    ...
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-3 sm:gap-4">
        {[...Array(5)].map((_, i) => (
          <div
            key={i}
            className="bg-card rounded-lg border border-border/40 p-4 sm:p-5 space-y-3"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 sm:gap-6">
        <div className="lg:col-span-2 bg-card rounded-lg border border-border/40 p-6 space-y-4">
          <div className="flex items-center justify-between">
            <Skeleton className="h-6 w-40" />
      ...
      <div className="bg-card rounded-lg border border-border/40 p-6 space-y-4">
        ...
        <div className="space-y-3">
          {[...Array(6)].map((_, i) => (
            <div key={i} className="flex items-center justify-between p-3 rounded-md border border-border/40">
              <div className="flex items-center gap-3">
...
export function SimpleCardSkeleton({ cards = 3 }: { cards?: number }) {
  return (
    <div className="max-w-3xl mx-auto space-y-6 py-8">
      ...
      {[...Array(cards)].map((_, s) => (
        <div key={s} className="bg-card rounded-lg border border-border/40 p-8 space-y-6">
          <div className="space-y-2">
            <Skeleton className="h-5 w-32" />
