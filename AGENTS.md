...
|-------|-----------|-------------|
| `orchestrator` | Team coordination | Entry point for all requests |
| `architect` | System design | New features, structural changes |
| `planner` | Task breakdown | After architecture approved |
| `builder` | Implementation | After planning complete |
| `tester` | Quality assurance | After implementation |
| `security` | Security review | After testing |
| `reviewer` | Code quality | After security review |
| `financial-auditor` | Financial verification | Conditional: monetary operations |
| `devops` | Deployment | Conditional: deployment needed |
| `documenter` | Documentation | Conditional: docs needed |

### Quality Gates

Every feature must pass through these gates in order:

1. **Architecture Gate** — Architect approves design
2. **Planning Gate** — Planner creates task breakdown
3. **Implementation Gate** — Builder completes code
4. **Testing Gate** — Tester verifies functionality
5. **Security Gate** — Security reviews for vulnerabilities
6. **Review Gate** — Reviewer approves code quality
7. **Financial Gate** — Financial auditor verifies calculations (if applicable)
8. **Deployment Gate** — DevOps guides deployment (if applicable)

### Non-Functional Requirements

All agents must enforce these requirements:

- **Security**: Never trust client input, validate on all layers, use parameterized queries
- **Validation**: Zod schemas, TypeScript strict mode, CHECK constraints in DB
- **Data Integrity**: FOREIGN KEY constraints, CHECK constraints for positive values, state machine validation
- **Performance**: Caching, timeouts, error boundaries, query optimization
- **Scalability**: SOLID principles, dependency injection, modular design

### Conditional Agents

| Agent | Condition |
|-------|-----------|
| `financial-auditor` | Feature involves prices, COD, settlements, financial reports |
| `devops` | Feature requires deployment or infrastructure changes |
| `documenter` | Feature requires documentation updates |

### Workflow

```
User Request → Orchestrator → Architect → Planner → Builder → Tester → Security → Reviewer → [Financial] → [DevOps] → [Documenter] → Complete
```
