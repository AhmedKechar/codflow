...
export async function getAllProfiles(db: AppDb, storeId: string): Promise<ShippingProfile[]> {
  const profiles = await db.select().from(shippingProfiles).where(eq(shippingProfiles.storeId, storeId)).all();
    ...
    profiles.map(async (p) => {
      const [rules, productRows] = await Promise.all([
        db
          ...
          .from(shippingRules)
          .where(and(eq(shippingRules.profileId, p.id), eq(shippingRules.storeId, storeId)))
          .all(),
        db
          .select({ id: products.id })
          .from(products)
          .where(and(eq(products.shippingProfileId, p.id), eq(products.storeId, storeId)))
          .all(),
      ]);
      return {
        id: p.id,
...
export async function getProfileById(
  ...
  const profile = await db
    ...
    .where(and(eq(shippingProfiles.id, id), eq(shippingProfiles.storeId, storeId)))
    .get();
  ...
  const [rules, productRows] = await Promise.all([
    db
      ...
      .where(and(eq(shippingRules.profileId, id), eq(shippingRules.storeId, storeId)))
      .all(),
    db
      .select({ id: products.id })
      .from(products)
      .where(and(eq(products.shippingProfileId, id), eq(products.storeId, storeId)))
      .all(),
  ]);
...
export async function getDefaultProfileRules(db: AppDb, storeId: string): Promise<ShippingRule[]> {
  ...
  const rules = await db
    ...
    .from(shippingRules)
    .leftJoin(wilayas, eq(shippingRules.wilayaId, wilayas.id))
    .where(and(eq(shippingRules.profileId, defaultProfile.id), eq(shippingRules.storeId, storeId)))
    .all();
...
export async function createProfile(
  ...
  if (data.isDefault) {
    await db.update(shippingProfiles).set({ isDefault: false }).where(eq(shippingProfiles.storeId, storeId)).run();
  }
...
export async function deleteProfile(db: AppDb, storeId: string, id: string): Promise<boolean> {
  ...
  await db.delete(shippingProfiles).where(and(eq(shippingProfiles.id, id), eq(shippingProfiles.storeId, storeId))).run();
  return true;
}
...
export async function getWilayaRule(
  ...
  return db
    ...
    .from(shippingRules)
    .where(
      and(eq(shippingRules.profileId, profileId), eq(shippingRules.wilayaId, wilayaId), eq(shippingRules.storeId, storeId)),
    )
    .get();
}
...
export async function getCommunesWithOverrides(
  ...
  const overrideRows = await db
    ...
    .where(and(eq(shippingRuleCommunes.ruleId, ruleId), eq(shippingRuleCommunes.storeId, storeId)))
    .all();
    ...
    await db
      ...
      .where(
        and(
          ...
          eq(shippingRuleCommunes.storeId, storeId),
        ),
      )
      .run();
    return;
