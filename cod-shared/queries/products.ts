...
async function buildProductDetail(db: AppDb, storeId: string, productId: string) {
  const product = await db.select().from(products).where(and(eq(products.id, productId), eq(products.storeId, storeId), isNull(products.deletedAt))).get();
  if (!product) return null;

  const [category, variants, images, totalInventoryRow] = await Promise.all([
    product.categoryId
      ? db.select().from(productCategories).where(and(eq(productCategories.id, product.categoryId), eq(productCategories.storeId, storeId))).get()
      : Promise.resolve(null),
    db.select().from(productVariants).where(and(eq(productVariants.productId, productId), eq(productVariants.storeId, storeId))).orderBy(productVariants.position).all(),
    db.select().from(productImages).where(and(eq(productImages.productId, productId), eq(productImages.storeId, storeId))).orderBy(productImages.position).all(),
    db.select({ total: sum(productVariants.inventory) }).from(productVariants).where(and(eq(productVariants.productId, productId), eq(productVariants.storeId, storeId))).get(),
  ]);
...
export async function getAllProducts(db: AppDb, storeId: string, filters?: ProductFilters) {
  const conditions: ReturnType<typeof eq>[] = [];
  conditions.push(eq(products.storeId, storeId) as any);
  ...
  return Promise.all(rows.map(async (p) => {
    const [variantsCountRow, totalInventoryRow, imagesRow, variantRows, reviewCountRow, avgRatingRow] = await Promise.all([
      db.select({ count: count() }).from(productVariants).where(and(eq(productVariants.productId, p.id), eq(productVariants.storeId, storeId))).get(),
      db.select({ total: sum(productVariants.inventory) }).from(productVariants).where(and(eq(productVariants.productId, p.id), eq(productVariants.storeId, storeId))).get(),
      db.select({ src: productImages.src }).from(productImages).where(and(eq(productImages.productId, p.id), eq(productImages.storeId, storeId))).orderBy(productImages.position).limit(1).get(),
      db.select().from(productVariants).where(and(eq(productVariants.productId, p.id), eq(productVariants.storeId, storeId))).orderBy(productVariants.position).all(),
...
export async function getProductById(db: AppDb, storeId: string, productId: string) {
  return buildProductDetail(db, storeId, productId);
}

export async function createProduct(db: AppDb, storeId: string, data: CreateProductData) {
  ...
  return buildProductDetail(db, storeId, id);
}

export async function updateProduct(db: AppDb, storeId: string, productId: string, data: UpdateProductData) {
...
export async function deleteProduct(db: AppDb, storeId: string, productId: string) {
  await db.update(products).set({ deletedAt: new Date().toISOString(), updatedAt: new Date().toISOString() }).where(and(eq(products.id, productId), eq(products.storeId, storeId)));
  return { success: true };
}

export async function getProductImages(db: AppDb, storeId: string, productId: string) {
  return db
    .select()
    .from(productImages)
    .where(and(eq(productImages.productId, productId), eq(productImages.storeId, storeId)))
