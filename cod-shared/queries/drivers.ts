...
export async function getAllDrivers(db: AppDb, storeId: string, filters?: DriverFilters) {
  const conditions = [eq(drivers.storeId, storeId)];
  ...
  if (filters?.wilayaId) {
    const coveringDriverIds = await db
      ...
      .where(and(eq(driverCompensations.storeId, storeId), eq(driverCompensations.wilayaId, filters.wilayaId)))
      .all();
...
export async function getDriverById(db: AppDb, storeId: string, driverId: string) {
  const driver = await db
    .select()
    .from(drivers)
    .where(and(eq(drivers.storeId, storeId), eq(drivers.id, driverId)))
    .get();
  ...
  const compStats = await db
    ...
    .from(driverCompensations)
    .where(and(eq(driverCompensations.storeId, storeId), eq(driverCompensations.driverId, driverId)))
    .get();
...
export async function createDriver(db: AppDb, storeId: string, data: CreateDriverData) {
  // Check for duplicate phone number
  const existingDriver = await db
    .select({ id: drivers.id, phone: drivers.phone })
    .from(drivers)
    .where(and(eq(drivers.storeId, storeId), eq(drivers.phone, data.phone)))
    .get();
  ...
  return getDriverById(db, storeId, id);
}

export async function updateDriver(db: AppDb, storeId: string, driverId: string, data: UpdateDriverData) {
  const existing = await getDriverById(db, storeId, driverId);
  ...
  return getDriverById(db, storeId, driverId);
}
...
export async function setCompensation(
  ...
  const existing = await db
    ...
    .where(
      and(
        eq(driverCompensations.storeId, storeId),
