...
export async function upsertAbandonedOrder(
  ...
  const row = await db
    ...
    .where(and(eq(abandonedOrders.sessionId, data.sessionId), eq(abandonedOrders.storeId, storeId)))
    .limit(1)
    .then((r) => r[0]);
...
export async function sweepPendingToAbandoned(db: AppDb, storeId: string): Promise<number> {
  const now = new Date().toISOString();
...
export async function listAbandonedOrders(
  ...
  const conditions = [eq(abandonedOrders.storeId, storeId)];
...
export async function getAbandonedOrderStats(db: AppDb, storeId: string) {
  const [totalRow, convertedRow, revenueRow] = await Promise.all([
    db
      .select({ count: sql<number>`count(*)` })
      .from(abandonedOrders)
      .where(and(eq(abandonedOrders.storeId, storeId), eq(abandonedOrders.status, "abandoned"))),
    db
      .select({ count: sql<number>`count(*)` })
      .from(abandonedOrders)
      .where(and(eq(abandonedOrders.storeId, storeId), eq(abandonedOrders.status, "converted"))),
    db
...
export async function updateAbandonedOrderStatus(
  ...
  await db
    ...
    .where(and(eq(abandonedOrders.id, id), eq(abandonedOrders.storeId, storeId)));
}

export async function deleteAbandonedOrder(db: AppDb, storeId: string, id: string): Promise<void> {
  await db.delete(abandonedOrders).where(and(eq(abandonedOrders.id, id), eq(abandonedOrders.storeId, storeId)));
}
