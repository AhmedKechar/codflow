...
export async function getStockHistory(
  ...
  const conditions = [eq(stockMovements.productId, productId), eq(stockMovements.storeId, storeId)];
...
export async function getStockOverview(db: AppDb, storeId: string): Promise<StockOverview> {
...
export async function getStockAlerts(
  ...
  const overview = await getStockOverview(db, storeId);
...
export async function updateVariantThreshold(
  ...
  const existing = await db
    ...
    .from(productVariants)
    .where(and(eq(productVariants.id, variantId), eq(productVariants.productId, productId), eq(productVariants.storeId, storeId)))
    .get();
  if (!existing) return false;
