...
export async function getAllCustomers(db: AppDb, storeId: string, filters?: CustomerFilters) {
  ...
  if (filters?.groupId) {
    const members = await db
      ...
      .where(and(eq(customerGroupMembers.groupId, filters.groupId), eq(customerGroups.storeId, storeId)))
  ...
  if (filters?.tagId) {
    const assigned = await db
      ...
      .where(and(eq(customerTagAssignments.tagId, filters.tagId), eq(customerTags.storeId, storeId)))
...
export async function getCustomerById(db: AppDb, storeId: string, customerId: string) {
...
export async function getCustomerByPhone(db: AppDb, storeId: string, phone: string) {
  return await db
    .select()
    .from(customers)
...
export async function createCustomer(db: AppDb, storeId: string, customerData: CreateCustomerData) {
  ...
  return getCustomerById(db, storeId, customerId);
}
...
export async function updateCustomer(
  ...
  await db.update(customers).set(updateData).where(and(eq(customers.id, customerId), eq(customers.storeId, storeId)));
  return getCustomerById(db, storeId, customerId);
}

export async function getOrdersByCustomerId(db: AppDb, storeId: string, customerId: string) {
...
export async function getCustomerGroupMemberships(db: AppDb, storeId: string, customerId: string) {
  return await db
    .select({
    ...
    .from(customerGroupMembers)
    .innerJoin(customerGroups, eq(customerGroups.id, customerGroupMembers.groupId))
    .where(and(eq(customerGroupMembers.customerId, customerId), eq(customerGroups.storeId, storeId)))
    .all();
}

export async function getCustomerTagMemberships(db: AppDb, storeId: string, customerId: string) {
  return await db
    .select({
      id: customerTags.id,
    ...
    .from(customerTagAssignments)
    .innerJoin(customerTags, eq(customerTags.id, customerTagAssignments.tagId))
    .where(and(eq(customerTagAssignments.customerId, customerId), eq(customerTags.storeId, storeId)))
    .all();
}
