...
export async function listOffers(db: AppDb, storeId: string) {
  const rows = await db
    .select()
    .from(offers)
...
export async function getOfferById(db: AppDb, storeId: string, id: string) {
...
export async function deleteOffer(db: AppDb, storeId: string, id: string) {
  await db.delete(offers).where(and(eq(offers.storeId, storeId), eq(offers.id, id)));
}
