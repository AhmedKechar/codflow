 ...
 */
export async function getDriverPayments(db: AppDb, storeId: string, driverId: string) {
  return db
    .select()
    .from(driverPayments)
    .where(and(eq(driverPayments.storeId, storeId), eq(driverPayments.driverId, driverId)))
 ...
 */
export async function getPendingSettlementOrders(db: AppDb, storeId: string, driverId: string) {
  return db
    .select()
    .from(orders)
    .where(
