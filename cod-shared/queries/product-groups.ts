...
export async function getAllGroups(db: AppDb, storeId: string, filters?: GroupFilters) {
  const conditions = [];
  conditions.push(eq(productCategories.storeId, storeId));
...
export async function getGroupById(db: AppDb, storeId: string, id: string) {
  const cat = await db.select().from(productCategories).where(and(eq(productCategories.id, id), eq(productCategories.storeId, storeId))).get();
  if (!cat) return null;
  const [childrenRows, productsCountRow] = await Promise.all([
    db.select().from(productCategories).where(and(eq(productCategories.parentId, id), eq(productCategories.storeId, storeId))).all(),
    db
      .select({ count: count() })
      .from(products)
      .where(and(
...
export async function createGroup(db: AppDb, storeId: string, data: CreateGroupData) {
  ...
  return getGroupById(db, storeId, id);
}

export async function updateGroup(db: AppDb, storeId: string, id: string, data: UpdateGroupData) {
  ...
  await db.update(productCategories).set(updates).where(and(eq(productCategories.id, id), eq(productCategories.storeId, storeId)));
  return getGroupById(db, storeId, id);
}

export async function deleteGroup(db: AppDb, storeId: string, id: string) {
  await db.delete(productCategories).where(and(eq(productCategories.id, id), eq(productCategories.storeId, storeId)));
  return { success: true };
}
