...
export const users = sqliteTable("users", {
  ...
  image: text("image"),
  role: text("role", { enum: ["admin", "staff", "super_admin"] })
    .notNull()
    .default("staff"),
 ...
 * Each store has its own branding, theme, and SEO settings.
 * In multi-tenant mode, multiple stores share the same database.
 */
 ...
 * Platform-level definitions, not per-store.
 */
export const plans = sqliteTable("plans", {
  ...
  maxTeamMembers: integer("max_team_members").default(2),
  maxAiCredits: integer("max_ai_credits").default(0),
...
/**
 * Store subscriptions to plans.
 * One active subscription per store at a time.
 */
export const subscriptions = sqliteTable("subscriptions", {
  ...
  planId: text("plan_id")
    .notNull()
    .references(() => plans.id),
  status: text("status", { enum: ["trialing", "active", "past_due", "canceled", "expired"] })
    .notNull()
    .default("trialing"),
  ...
  paymentMethod: text("payment_method", { enum: ["ccp", "baridi_mob", "wise", "redotpay"] }),
  notes: text("notes"),
  createdAt: text("created_at").notNull().default(sql`datetime('now')`),
  updatedAt: text("updated_at").notNull().default(sql`datetime('now')`),
}, (t) => ({
  storeIdx: index("idx_subscriptions_store").on(t.storeId),
  statusIdx: index("idx_subscriptions_status").on(t.status),
}));

/**
 * Payment records for subscriptions.
 * Manual receipt upload with Super Admin approval.
 */
export const payments = sqliteTable("payments", {
  ...
  currency: text("currency").notNull().default("DZD"),
  paymentMethod: text("payment_method", { enum: ["ccp", "baridi_mob", "wise", "redotpay"] }).notNull(),
  ...
  storeIdx: index("idx_payments_store").on(t.storeId),
  subscriptionIdx: index("idx_payments_subscription").on(t.subscriptionId),
  statusIdx: index("idx_payments_status").on(t.status),
}));

/**
 * User-store junction table.
 * Maps users to stores with per-store roles and status.
 */
export const storeMembers = sqliteTable("store_members", {
  ...
  userIdx: index("idx_store_members_user").on(t.userId),
  storeIdx: index("idx_store_members_store").on(t.storeId),
  userStoreUnique: uniqueIndex("idx_store_members_user_store").on(t.userId, t.storeId),
}));
...
export const invitations = sqliteTable("invitations", {
  ...
  storeEmailUnique: uniqueIndex("idx_invitations_store_email").on(t.storeId, t.email),
  tokenUnique: uniqueIndex("idx_invitations_token").on(t.token),
}));
...
export const giftCards = sqliteTable("gift_cards", {
  ...
  storeId: text("store_id")
    ...
    .references(() => stores.id, { onDelete: "cascade" }),
  code: text("code").notNull(),
  initialAmountDzd: integer("initial_amount_dzd").notNull(),
  remainingAmountDzd: integer("remaining_amount_dzd").notNull(),
  status: text("status", { enum: ["active", "used", "expired", "disabled"] }).notNull().default("active"),
  ...
  storeIdx: index("idx_gift_cards_store").on(t.storeId),
  storeCodeUnique: uniqueIndex("idx_gift_cards_store_code").on(t.storeId, t.code),
}));
...
export const discountCodes = sqliteTable("discount_codes", {
  ...
  storeIdx: index("idx_discount_codes_store").on(t.storeId),
  storeCodeUnique: uniqueIndex("idx_discount_codes_store_code").on(t.storeId, t.code),
}));

/**
 * AI credits balance per store.
 */
export const aiCredits = sqliteTable("ai_credits", {
  ...
  storeIdx: uniqueIndex("idx_ai_credits_store").on(t.storeId),
}));

/**
 * AI credit usage history per store.
 */
export const aiCreditUsage = sqliteTable("ai_credit_usage", {
  id: text("id").primaryKey(),
  storeId: text("store_id")
    .notNull()
    .references(() => stores.id, { onDelete: "cascade" }),
  agentType: text("agent_type", { enum: ["marketing", "finance", "design", "branding", "product"] }).notNull(),
  ...
  storeIdx: index("idx_ai_credit_usage_store").on(t.storeId),
  storeAgentIdx: index("idx_ai_credit_usage_store_agent").on(t.storeId, t.agentType),
}));

/**
 * WhatsApp messages sent from stores.
 */
export const whatsappMessages = sqliteTable("whatsapp_messages", {
  ...
  messageType: text("message_type", { enum: ["order_update", "marketing", "support", "automated"] }).notNull(),
  ...
  storeIdx: index("idx_whatsapp_messages_store").on(t.storeId),
  storeOrderIdx: index("idx_whatsapp_messages_store_order").on(t.storeId, t.orderId),
}));

/**
 * SMS messages sent from stores.
 */
export const smsMessages = sqliteTable("sms_messages", {
  ...
  storeIdx: index("idx_sms_messages_store").on(t.storeId),
}));
...
export const customDomains = sqliteTable("custom_domains", {
  ...
  status: text("status", { enum: ["pending", "verifying", "active", "failed", "expired"] }).notNull().default("pending"),
  ...
  storeIdx: index("idx_custom_domains_store").on(t.storeId),
  domainUnique: uniqueIndex("idx_custom_domains_domain").on(t.domain),
}));

/**
 * Provider API keys managed by Super Admin.
 * All keys are stored securely and can be managed from the Super Admin dashboard.
 */
export const providerApiKeys = sqliteTable("provider_api_keys", {
  ...
  providerNameUnique: uniqueIndex("idx_provider_api_keys_provider_name").on(t.provider, t.keyName),
}));

/**
 * Automation workflows for stores.
 * Visual workflow builder for WhatsApp, email, and SMS automation.
 */
export const automationWorkflows = sqliteTable("automation_workflows", {
  ...
  name: text("name").notNull(),
  description: text("description"),
  triggerType: text("trigger_type", { enum: ["order_created", "order_status_changed", "abandoned_cart", "customer_signup", "custom"] }).notNull(),
  triggerConfig: text("trigger_config"), // JSON config
