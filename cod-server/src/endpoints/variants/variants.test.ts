...
describe("getVariantById", () => {
  it("returns null when variant does not exist", async () => {
    ...
    const result = await getVariantById(db as any, "test-store", "var_missing");
    expect(result).toBeNull();
  });

  it("returns the variant when found", async () => {
    const db = makeMockDb([f(variantRow())]);
    const result = await getVariantById(db as any, "test-store", "var_1");
...
describe("createVariant", () => {
  it("creates and returns the new variant", async () => {
    ...
    const result = await createVariant(db as any, "test-store", "prod_1", {
...
describe("updateVariant", () => {
  it("updates and returns the variant", async () => {
    ...
    const result = await updateVariant(db as any, "test-store", "var_1", { inventory: 20 });
    expect(result).not.toBeNull();
  ...
  it("returns variant after price update", async () => {
    ...
    const result = await updateVariant(db as any, "test-store", "var_1", { price: 1800 });
...
describe("deleteVariant", () => {
  it("nulls out order references and succeeds when variant is in orders", async () => {
    ...
    const db = makeMockDb([]);
    const result = await deleteVariant(db as any, "test-store", "var_1");
