...
export async function listGroups(c: Context<AppContext>) {
  ...
  const groups = await queries.getAllGroups(db, storeId, filters);
...
export async function getGroup(c: Context<AppContext>) {
  ...
  const group = withMembers
    ? await queries.getGroupWithMembers(db, storeId, groupId)
    : await queries.getGroupById(db, storeId, groupId);
  if (!group) {
...
export async function createGroup(c: Context<AppContext>) {
  ...
  const group = await queries.createGroup(db, storeId, validated);
  if (!group) {
...
export async function updateGroup(c: Context<AppContext>) {
  ...
  const group = await queries.updateGroup(db, storeId, groupId, validated);
  if (!group) {
    throw new NotFoundError("customer_group", groupId);
  }
  const actor = c.get("user");
...
export async function deleteGroup(c: Context<AppContext>) {
  ...
  const storeId = c.get("storeId")!;
  const groupId = c.req.param("id")!;
  const group = await queries.getGroupById(db, storeId, groupId);
  if (!group) {
    throw new NotFoundError("customer_group", groupId);
  }
  ...
  await queries.deleteGroup(db, storeId, groupId);
  const actor = c.get("user");
...
export async function addMember(c: Context<AppContext>) {
  ...
  await queries.addMember(db, storeId, groupId, customerId);
  const actor = c.get("user");
...
export async function removeMember(c: Context<AppContext>) {
  ...
  await queries.removeMember(db, storeId, groupId, customerId);
  const actor = c.get("user");
