...
describe("getCustomerById", () => {
  it("returns null when customer does not exist", async () => {
    ...
    const result = await getCustomerById(db, "test-store", "cust_missing");
  ...
  it("returns the customer with recentOrders when found", async () => {
    ...
    const result = await getCustomerById(db, "test-store", "cust_1");
...
describe("createCustomer", () => {
  it("creates a customer successfully", async () => {
    ...
    const result = await createCustomer(db, "test-store", {
      name: "Omar Khaled",
      phone: "0551234567",
      wilayaId: 31,
    });
...
describe("deleteCustomer", () => {
  it("throws when customer has existing orders", async () => {
    ...
    await expect(deleteCustomer(db, "test-store", "cust_1")).rejects.toThrow(
  ...
  it("succeeds when customer has no orders", async () => {
    ...
    await expect(deleteCustomer(db, "test-store", "cust_1")).resolves.toEqual({ success: true });
  });
});
