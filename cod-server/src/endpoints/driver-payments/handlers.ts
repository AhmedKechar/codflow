...
export async function createPayment(c: Context<AppContext>) {
  ...
  const payment = await queries.createDriverPayment(db, storeId, validated, user.id, user.name ?? user.id);
  console.info(
...
export async function listPayments(c: Context<AppContext>) {
  ...
  const data = await queries.getDriverPayments(db, storeId, driverId);
...
export async function listPendingOrders(c: Context<AppContext>) {
  ...
  const data = await queries.getPendingSettlementOrders(db, storeId, driverId);
  return c.json({ success: true, data }, 200);
}
