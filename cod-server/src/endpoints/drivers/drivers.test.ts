...
describe("getDriverById", () => {
  it("returns null when driver does not exist", async () => {
    ...
    const result = await getDriverById(db, "test-store", "drv_missing");
  ...
  it("returns the driver with compensation summary when found", async () => {
    ...
    const result = await getDriverById(db, "test-store", "drv_1");
...
describe("updateDriverStatus", () => {
  it("returns null when driver does not exist", async () => {
    const db = makeMockDb([f(null)]);
    const result = await updateDriverStatus(db, "test-store", "drv_missing", "busy");
    expect(result).toBeNull();
  });
  ...
  it("succeeds for an existing driver", async () => {
    ...
    const result = await updateDriverStatus(db, "test-store", "drv_1", "busy");
    expect(result).not.toBeNull();
  });
});
