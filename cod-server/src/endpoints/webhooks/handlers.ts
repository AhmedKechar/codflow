...
export async function handleZrWebhook(c: Context<AppContext>) {
  ...
  const { id: webhookEventId, isDuplicate } = await insertWebhookEvent(db, storeId, {
    provider: "zr_express",
    eventId,
  ...
  try {
    ...
    const order = trackingNumber
      ? await getOrderByTracking(db, storeId, trackingNumber)
      : null;
    const resolvedOrder = order ?? (externalId ? await getOrderByReference(db, storeId, externalId) : null);
