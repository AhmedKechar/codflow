...
describe("getProductById", () => {
  it("returns null when product is not found (or soft-deleted)", async () => {
    ...
    const result = await getProductById(db, "test-store", "prod_missing");
  ...
  it("returns the product when found", async () => {
    ...
    const result = await getProductById(db, "test-store", "prod_1");
...
describe("deleteProduct", () => {
  it("soft-deletes the product (returns success, does not hard delete)", async () => {
    ...
    const db = makeMockDb([]);
    const result = await deleteProduct(db, "test-store", "prod_1");
...
describe("createProduct", () => {
  it("auto-generates handle when not provided", async () => {
    ...
    const result = await createProduct(db, "test-store", {
      name: "T-Shirt",
      price: 1500,
      type: "PHYSICAL",
      hasVariants: false,
