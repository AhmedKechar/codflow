...
export async function listProducts(c: Context<AppContext>) {
  ...
  const data = await queries.getAllProducts(db, storeId, filters);
...
export async function getProduct(c: Context<AppContext>) {
  ...
  const product = await queries.getProductById(db, storeId, productId);
  if (!product) {
...
export async function createProduct(c: Context<AppContext>) {
  ...
  if (validated.sku) {
    ...
    const existingProduct = await db.select().from(products)
      .where(and(eq(products.sku, validated.sku), eq(products.storeId, storeId), isNull(products.deletedAt)))
      .get();
    if (existingProduct) {
  ...
  const product = await queries.createProduct(db, storeId, validated);
  if (!product) {
...
export async function updateProduct(c: Context<AppContext>) {
  ...
  const product = await queries.updateProduct(db, storeId, productId, validated);
  if (!product) {
    throw new NotFoundError("Product", productId);
  }
  const actor = c.get("user");
...
export async function updateProductStatus(c: Context<AppContext>) {
  ...
  const product = await queries.updateProduct(db, storeId, productId, { status });
  if (!product) {
    throw new NotFoundError("Product", productId);
  }
  const actor = c.get("user");
...
export async function deleteProduct(c: Context<AppContext>) {
  ...
  const existing = await queries.getProductById(db, storeId, id);
  if (!existing) {
  ...
  await queries.deleteProduct(db, storeId, id);
  const actor = c.get("user");
