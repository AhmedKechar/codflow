...
describe("getProfileById", () => {
  it("returns null when profile does not exist", async () => {
    ...
    const result = await getProfileById(db as any, "test-store", "prof_missing");
  ...
  it("returns profile with empty rules and zero counts when no associations", async () => {
    ...
    const result = await getProfileById(db as any, "test-store", "prof_1");
...
describe("createProfile", () => {
  it("creates and returns the profile without DB reads", async () => {
    ...
    const result = await createProfile(db as any, "test-store", { name: "Express DZ", isDefault: false });
  ...
  it("returns isDefault = true when specified", async () => {
    const db = makeMockDb([]);
    const result = await createProfile(db as any, "test-store", { name: "Default Profile", isDefault: true });
    expect(result.isDefault).toBe(true);
  });

  it("accepts optional notes", async () => {
    const db = makeMockDb([]);
    const result = await createProfile(db as any, "test-store", { name: "Express DZ", isDefault: false, notes: "ملاحظة" });
...
describe("deleteProfile", () => {
  it("returns false when profile does not exist", async () => {
    const db = makeMockDb([f(null)]);
    const result = await deleteProfile(db as any, "test-store", "prof_missing");
    expect(result).toBe(false);
  });

  it("returns true when profile is found and deleted", async () => {
    const db = makeMockDb([f(shippingProfileRow())]);
    const result = await deleteProfile(db as any, "test-store", "prof_1");
