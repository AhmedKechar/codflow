...
export async function listProfiles(c: Context<AppContext>) {
  ...
  const data = await queries.getAllProfiles(db, storeId);
...
export async function getDefaultRules(c: Context<AppContext>) {
  const db = getDb(c.env.DB);
  const storeId = c.get("storeId")!;
  const data = await queries.getDefaultProfileRules(db, storeId);
...
export async function getProfile(c: Context<AppContext>) {
  const db = getDb(c.env.DB);
  const storeId = c.get("storeId")!;
  const profile = await queries.getProfileById(db, storeId, c.req.param("id")!);
  if (!profile) {
...
export async function createProfile(c: Context<AppContext>) {
  ...
  const profile = await queries.createProfile(db, storeId, body);
...
export async function deleteProfile(c: Context<AppContext>) {
  ...
  const profile = await queries.getProfileById(db, storeId, id);
  if (!profile) {
    throw new NotFoundError("shipping_profile", id);
  }
  ...
  await queries.deleteProfile(db, storeId, id);
...
export async function listCommuneOverrides(c: Context<AppContext>) {
  ...
  const rule = await queries.getWilayaRule(db, storeId, profileId, wilayaId);
  if (!rule) {
    throw new NotFoundError("shipping_rule", `profile=${profileId} wilaya=${wilayaId}`);
  }

  const communes = await queries.getCommunesWithOverrides(db, storeId, rule.id, wilayaId, {
...
export async function setCommuneOverride(c: Context<AppContext>) {
  ...
  await queries.setCommuneOverride(db, storeId, rule.id, communeId, body);
...
export async function deleteCommuneOverride(c: Context<AppContext>) {
  ...
  const deleted = await queries.deleteCommuneOverride(db, storeId, rule.id, communeId);
  if (!deleted) {
