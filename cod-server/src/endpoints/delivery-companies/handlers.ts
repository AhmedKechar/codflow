...
export async function listDeliveryCompanies(c: Context<AppContext>) {
  ...
  const data = await queries.getAllDeliveryCompanies(db, storeId, parsed);
...
export async function getDeliveryCompany(c: Context<AppContext>) {
  ...
  const company = await queries.getDeliveryCompanyById(db, storeId, id);
...
export async function createDeliveryCompany(c: Context<AppContext>) {
  ...
  const existing = await queries.getDeliveryCompanyByCode(db, storeId, parsed.code);
  if (existing) {
  ...
  const company = await queries.createDeliveryCompany(db, storeId, { ...parsed, autoValidate });
  if (!company) {
...
export async function updateDeliveryCompany(c: Context<AppContext>) {
  ...
  const company = await queries.updateDeliveryCompany(db, storeId, id, parsed);
...
export async function deleteDeliveryCompany(c: Context<AppContext>) {
  ...
  const existing = await queries.getDeliveryCompanyById(db, storeId, id);
  ...
  try {
    await queries.deleteDeliveryCompany(db, storeId, id);
  } catch (err) {
    throw new BusinessLogicError(
