...
export async function listTags(c: Context<AppContext>) {
  ...
  const tags = await queries.getAllTags(db, storeId, filters);
...
export async function getTag(c: Context<AppContext>) {
  ...
  const tag = withCustomers
    ? await queries.getTagWithCustomers(db, storeId, tagId)
    : await queries.getTagById(db, storeId, tagId);
  if (!tag) {
...
export async function createTag(c: Context<AppContext>) {
  ...
  const tag = await queries.createTag(db, storeId, validated);
  if (!tag) {
...
export async function updateTag(c: Context<AppContext>) {
  ...
  const tag = await queries.updateTag(db, storeId, tagId, validated);
  if (!tag) {
    throw new NotFoundError("customer_tag", tagId);
  }
  const actor = c.get("user");
...
export async function deleteTag(c: Context<AppContext>) {
  ...
  const storeId = c.get("storeId")!;
  const tagId = c.req.param("id")!;
  const tag = await queries.getTagById(db, storeId, tagId);
  if (!tag) {
    throw new NotFoundError("customer_tag", tagId);
  }
  ...
  await queries.deleteTag(db, storeId, tagId);
  const actor = c.get("user");
...
export async function assignTag(c: Context<AppContext>) {
  ...
  await queries.assignTag(db, storeId, tagId, customerId);
  const actor = c.get("user");
...
export async function unassignTag(c: Context<AppContext>) {
  ...
  await queries.unassignTag(db, storeId, tagId, customerId);
  const actor = c.get("user");
