 ...
 */
export const getOfferTools = (db: ReturnType<typeof getDb>, storeId: string) => ({
  ...
  listOffers: tool({
    ...
    execute: async (_args) => {
      ...
      try {
        const offerList = await queries.listOffers(db, storeId);
        return {
          success: true,
  ...
  getOfferDetails: tool({
    ...
    execute: async (args) => {
      ...
      try {
        const offer = await queries.getOfferById(db, storeId, parsed.data.offerId);
        if (!offer) {
          return {
            success: false,
  ...
  createOffer: tool({
    ...
    execute: async (args) => {
      ...
      try {
        const { id } = await queries.createOffer(db, storeId, parsed.data);
        const offer = await queries.getOfferById(db, storeId, id);
        return {
          success: true,
          offer,
  ...
  updateOffer: tool({
    ...
    execute: async (args) => {
      ...
      try {
        const existing = await queries.getOfferById(db, storeId, parsed.data.offerId);
        if (!existing) {
          return {
            success: false,
            error: `Offer not found with ID: ${parsed.data.offerId}`,
        ...
        await queries.updateOffer(db, storeId, parsed.data.offerId, parsed.data.updates);
        const offer = await queries.getOfferById(db, storeId, parsed.data.offerId);
        return {
          success: true,
          offer,
  ...
  deleteOffer: tool({
    ...
    execute: async (args) => {
      ...
      try {
        ...
        await queries.deleteOffer(db, storeId, parsed.data.offerId);
        return {
          success: true,
