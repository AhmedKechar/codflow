...
describe("getOfferById", () => {
  it("returns null when offer doesn't exist", async () => {
    ...
    const result = await getOfferById(db, "test-store", "nonexistent");
  ...
  it("returns offer with resolved product references", async () => {
    ...
    const result = await getOfferById(db, "test-store", "offer_1");
...
describe("createOffer", () => {
  it("creates offer successfully with valid data", async () => {
    ...
    const result = await createOffer(db, "test-store", {
      name: "Buy 2 Get 1 Free",
      discountType: "free",
      triggerProductId: "prod_1",
      triggerQuantity: 2,
...
describe("updateOffer", () => {
  it("updates offer successfully", async () => {
    ...
    await expect(
      updateOffer(db, "test-store", "offer_1", {
...
describe("deleteOffer", () => {
  it("deletes offer successfully", async () => {
    ...
    await expect(deleteOffer(db, "test-store", "offer_1")).resolves.not.toThrow();
  });
});
