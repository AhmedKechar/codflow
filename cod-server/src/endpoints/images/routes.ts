...
// ─── Upload (multipart/form-data — needs custom responses) ───────────────────
...
const uploadImageRoute = defineRoute({
  ...
  responses: {
    201: {
      ...
      content: {
        "application/json": {
          schema: SuccessResponseSchema(UploadedImageSchema),
        },
      },
    },
    400: errorResponse(
...
// ─── Presign (JSON body — standard pattern) ─────────────────────────────────
...
const presignRoute = defineRoute({
  ...
  responses: {
    200: {
      ...
      content: {
        "application/json": {
          schema: SuccessResponseSchema(PresignedUploadSchema),
        },
      },
    },
...
const uploadRouter = new OpenAPIHono<AppContext>();
...
// ─── Serve (no auth, mounted outside /api/*) ────────────────────────────────
