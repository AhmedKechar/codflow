...
describe("createOrder", () => {
  ...
  it("handles zero-product order (no inventory loop runs)", async () => {
    ...
    await expect(
      createOrder(db, "test-store", baseOrder, [], null)
    ).resolves.toBe("ord_1");
  });
});
...
describe("updateOrderStatus", () => {
  ...
  it("restocks simple product inventory when cancelled", async () => {
    ...
    const result = await updateOrderStatus(db, "test-store", "ord_1", "cancelled", "user_1", "Admin");
  ...
  it("restocks correct remaining quantity when some units already returned", async () => {
    ...
    await updateOrderStatus(db, "test-store", "ord_1", "returned");
  ...
  it("skips restock when all units already returned (remaining = 0)", async () => {
    ...
    const result = await updateOrderStatus(db, "test-store", "ord_1", "cancelled");
...
describe("setOrderProductReturn", () => {
  it("returns 'returned' status when all units returned", async () => {
    ...
    const result = await setOrderProductReturn(db, "test-store", "ord_1", "op_1", 2);
  ...
  it("returns 'partially_returned' when only some units returned", async () => {
    ...
    const result = await setOrderProductReturn(db, "test-store", "ord_1", "op_1", 1);
  ...
  it("returns 'fulfilled' when returnedQuantity is set to 0", async () => {
    ...
    const result = await setOrderProductReturn(db, "test-store", "ord_1", "op_1", 0);
  ...
  it("restores inventory by the delta amount, not the full quantity", async () => {
    ...
    const result = await setOrderProductReturn(db, "test-store", "ord_1", "op_1", 4);
  ...
  it("handles variant inventory path", async () => {
    ...
    const result = await setOrderProductReturn(db, "test-store", "ord_1", "op_1", 3, "user_1", "Admin");
  ...
  it("throws when product line is not found", async () => {
    ...
    await expect(
      setOrderProductReturn(db, "test-store", "ord_1", "op_missing", 1)
  ...
  it("throws when returnedQuantity exceeds line quantity", async () => {
    ...
    await expect(
      setOrderProductReturn(db, "test-store", "ord_1", "op_1", 5) // 5 > 2
  ...
  it("throws when returnedQuantity is negative", async () => {
    ...
    await expect(
      setOrderProductReturn(db, "test-store", "ord_1", "op_1", -1)
...
describe("unassignDriver", () => {
  ...
  it("handles order not found gracefully (null status)", async () => {
    ...
    const result = await unassignDriver(db, "test-store", "ord_missing");
...
describe("deleteOrder", () => {
  it("completes without error for a basic order with no products", async () => {
    ...
    await expect(deleteOrder(db, "test-store", "ord_1")).resolves.toBeUndefined();
  });
  ...
  it("handles order not found gracefully — skips customer update", async () => {
    ...
    await expect(deleteOrder(db, "test-store", "ord_missing")).resolves.toBeUndefined();
