...
describe("Reviews routes (OpenAPIHono)", () => {
  ...
  describe("GET /api/reviews", () => {
    ...
    it("passes query parameters (status, productId, limit, offset) to queries.getAllReviews", async () => {
      ...
      expect(res.status).toBe(200);
      expect(queries.getAllReviews).toHaveBeenCalledWith(mockDb, "test-store", {
        status: "approved",
        productId: "prod_1",
        limit: 10,
        offset: 5,
  ...
  describe("PATCH /api/reviews/:id", () => {
    it("updates review status successfully", async () => {
      ...
      expect(queries.updateReviewStatus).toHaveBeenCalledWith(mockDb, "test-store", "rev_1", "approved");
    });
  ...
  describe("DELETE /api/reviews/:id", () => {
    it("deletes review successfully", async () => {
      ...
      expect(queries.deleteReview).toHaveBeenCalledWith(mockDb, "test-store", "rev_1");
    });
