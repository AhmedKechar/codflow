...
export async function listReviews(c: Context<AppContext>) {
  ...
  const { rows, total, pendingCount } = await queries.getAllReviews(db, storeId, filters);
...
export async function updateReview(c: Context<AppContext>) {
  ...
  const existing = await queries.getReviewById(db, storeId, id);
  ...
  const updated = await queries.updateReviewStatus(db, storeId, id, data.status);
...
export async function deleteReview(c: Context<AppContext>) {
  ...
  await queries.deleteReview(db, storeId, id);
