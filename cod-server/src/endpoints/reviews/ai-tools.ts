...
export const getReviewTools = (db: ReturnType<typeof getDb>, storeId: string) => ({
  ...
  listReviews: tool({
    ...
    execute: async (args) => {
      ...
      try {
        const { rows, total, pendingCount } = await queries.getAllReviews(db, storeId, parsed.data);
        return {
          success: true,
          count: rows.length,
          total,
  ...
  moderateReview: tool({
    ...
    execute: async (args) => {
      ...
      try {
        const existing = await queries.getReviewById(db, storeId, parsed.data.reviewId);
        if (!existing) {
          return {
            success: false,
        ...
        const updated = await queries.updateReviewStatus(db, storeId, parsed.data.reviewId, parsed.data.status);
        return {
          success: true,
          review: updated,
  ...
  deleteReview: tool({
    ...
    execute: async (args) => {
      ...
      try {
        ...
        await queries.deleteReview(db, storeId, parsed.data.reviewId);
        return {
          success: true,
