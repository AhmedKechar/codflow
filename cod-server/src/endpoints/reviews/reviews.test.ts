...
describe("getReviewById", () => {
  it("returns undefined when review doesn't exist", async () => {
    ...
    const result = await getReviewById(db, "test-store", "nonexistent");
    expect(result).toBeUndefined();
  });

  it("returns review when it exists", async () => {
    const db = makeMockDb([a([reviewRow()])]);
    const result = await getReviewById(db, "test-store", "review_1");
...
describe("updateReviewStatus", () => {
  it("updates review status successfully", async () => {
    ...
    const result = await updateReviewStatus(db, "test-store", "review_1", "approved");
    expect(result).toBeDefined();
  ...
  it("updates review status to rejected", async () => {
    ...
    const result = await updateReviewStatus(db, "test-store", "review_1", "rejected");
    expect(result).toBeDefined();
  ...
  it("updates review status back to pending", async () => {
    ...
    const result = await updateReviewStatus(db, "test-store", "review_1", "pending");
    expect(result).toBeDefined();
    expect(result?.status).toBe("pending");
  });
});
...
describe("deleteReview", () => {
  it("deletes review successfully", async () => {
    const db = makeMockDb([]);
    await expect(deleteReview(db, "test-store", "review_1")).resolves.not.toThrow();
  });
});
