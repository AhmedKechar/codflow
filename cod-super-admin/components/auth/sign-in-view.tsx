...
import { useAuth, useNavigation } from "@/lib/translations";
...
import { Lock, Mail, ShieldCheck, AlertCircle, Loader2, Sparkles, Command } from "lucide-react";
...
export function SignInView() {
  ...
  async function handleMagicLinkRequest(e: FormEvent) {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : auth.magic_link_error_generic);
    } finally {
      ...
      <div className="w-full max-w-[420px] z-10 space-y-8 animate-fade-in-up">
        <div className="flex flex-col items-center gap-4 text-center">
          ...
          <div className="space-y-1.5">
            ...
            <p className="text-muted-foreground text-sm font-medium">{auth.subtitle}</p>
          </div>
        </div>
          ...
          <CardContent className="pt-6">
            {magicLinkSent ? (
              ...
              <form onSubmit={handleMagicLinkRequest} className="space-y-4" dir={dir}>
                <div className="space-y-2">
                  ...
                  <div className="relative">
                    ...
                    <Input
                      ...
                      className={cn(dir === "rtl" ? "pr-10 pl-4" : "pl-10 pr-4")}
                ...
                <Button type="submit" disabled={magicLinkLoading} className="w-full h-12 rounded-xl text-sm font-black tracking-wide shadow-glow">
                  {magicLinkLoading ? (
              ...
              <form onSubmit={handleSubmit} className="space-y-4" dir={dir}>
                ...
                <Button type="submit" disabled={loading} className="w-full h-12 rounded-xl text-sm font-black tracking-wide shadow-glow transition-all active:scale-[0.98]">
                  {loading ? (
                    <Loader2 className="w-4 h-4 animate-spin" />
                  ) : (
                    <>
                ...
                <div className="relative my-6">
                  ...
                  <div className="relative flex justify-center text-xs uppercase">
                    <span className="bg-card px-2 text-muted-foreground font-bold">{auth.or}</span>
                  </div>
                </div>
